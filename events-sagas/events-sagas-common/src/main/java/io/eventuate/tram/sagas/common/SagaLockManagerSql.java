...
public class SagaLockManagerSql {
  ...
  public SagaLockManagerSql(EventsSchema eventsSchema) {

    String sagaLockTable = eventsSchema.qualifyTable("saga_lock_table");
    String sagaStashTable = eventsSchema.qualifyTable("saga_stash_table");

    insertIntoSagaLockTableSql =
        String.format("INSERT INTO %s(target, saga_type, saga_id) VALUES(?, ?,?)", sagaLockTable);
    ...
    updateSagaLockTableSql =
        String.format("update %s set saga_type = ?, saga_id = ? where target = ?", sagaLockTable);
