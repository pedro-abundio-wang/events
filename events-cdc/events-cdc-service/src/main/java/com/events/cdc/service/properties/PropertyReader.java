...
public class PropertyReader {
  ...
  public <PROPERTIES> PROPERTIES convertMapToPropertyClass(
      Map<String, Object> properties, Class<PROPERTIES> propertyClass) {
  ...
  public void checkForUnknownProperties(Map<String, Object> properties, Class<?> propertyClass) {
    List<String> propNames =
        Arrays.stream(BeanUtils.getPropertyDescriptors(propertyClass))
    ...
    if (!unexpectedProperties.isEmpty()) {
      String unexpectedPropertiesString =
          unexpectedProperties.stream().collect(Collectors.joining(", ", "[", "]"));

      throw new RuntimeException(
          String.format("Unknown properties: %s", unexpectedPropertiesString));
    }
  }
}
