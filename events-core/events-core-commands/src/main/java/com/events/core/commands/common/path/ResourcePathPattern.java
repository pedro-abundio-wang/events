...
public class ResourcePathPattern {
  ...
  public boolean isSatisfiedBy(ResourcePath mr) {
    if (splits.length != mr.splits.length) return false;
    for (int i = 0; i < mr.splits.length; i++)
      if (!pathSegmentMatches(splits[i], mr.splits[i])) return false;
    return true;
  }
    ...
    return new ResourcePath(
        Arrays.stream(splits)
            .map(
                s ->
                    isPlaceholder(s)
                        ? getPlaceholderValue(pathParams, idx.getAndIncrement())
                            .orElseGet(
                                () -> {
                                  throw new RuntimeException(
                                      "Placeholder not found: "
