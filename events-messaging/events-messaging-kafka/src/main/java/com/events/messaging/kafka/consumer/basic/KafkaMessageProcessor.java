...
public class KafkaMessageProcessor {
  ...
  private final BlockingQueue<ConsumerRecord<String, byte[]>> processedRecords =
      new LinkedBlockingQueue<>();
  private final AtomicReference<KafkaMessageProcessorFailedException> failed =
      new AtomicReference<>();
  private final Set<MessageConsumerBacklog> consumerBacklogs = new HashSet<>();

  public KafkaMessageProcessor(String subscriberId, KafkaMessageConsumerHandler handler) {
  ...
  public void process(ConsumerRecord<String, byte[]> record) {
    ...
    MessageConsumerBacklog consumerBacklog =
        handler.apply(
            record,
            (result, t) -> {
              if (t != null) {
                ...
                logger.debug(
                    "Adding processed record to queue {} {}", subscriberId, record.offset());
                processedRecords.add(record);
              }
            });
    if (consumerBacklog != null) consumerBacklogs.add(consumerBacklog);
  }
