...
import com.events.messaging.kafka.common.sbe.MessageHeaderDecoder;
import com.events.messaging.kafka.common.sbe.MessageHeaderEncoder;
import com.events.messaging.kafka.common.sbe.MultiMessageDecoder;
...
public class KafkaMultiMessageConverter {
  ...
  public byte[] convertMessagesToBytes(List<KafkaMultiMessage> kafkaMultiMessages) {
    return convertMessagesToBytes(
        new KafkaMultiMessages(Collections.emptyList(), kafkaMultiMessages));
  }

  public byte[] convertMessagesToBytes(KafkaMultiMessages kafkaMultiMessages) {

    MessageBuilder builder = new MessageBuilder();

    builder.setHeaders(kafkaMultiMessages.getHeaders());

    for (KafkaMultiMessage message : kafkaMultiMessages.getMessages()) {
  ...
  public KafkaMultiMessages convertBytesToMessages(byte[] bytes) {
    ...
    MultiMessageDecoder multiMessageDecoder =
        new MultiMessageDecoder()
            .wrap(buffer, messageHeaderDecoder.encodedLength(), actingBlockLength, actingVersion);

    List<KafkaMultiMessagesHeader> headers =
        decodeEventsKafkaMultiMessagesHeaders(multiMessageDecoder);
    List<KafkaMultiMessage> messages = decodeEventsKafkaMultiMessages(multiMessageDecoder);

    return new KafkaMultiMessages(headers, messages);
  }

  private List<KafkaMultiMessagesHeader> decodeEventsKafkaMultiMessagesHeaders(
      MultiMessageDecoder multiMessageDecoder) {
  ...
  private List<KafkaMultiMessage> decodeEventsKafkaMultiMessages(
      MultiMessageDecoder multiMessageDecoder) {
    ...
    for (int i = 0; i < messagesDecoder.count(); i++) {
      ...
      List<KafkaMultiMessageHeader> messageHeaders =
          decodeEventsKafkaMultiMessageHeaders(messagesDecoder);
  ...
  private List<KafkaMultiMessageHeader> decodeEventsKafkaMultiMessageHeaders(
