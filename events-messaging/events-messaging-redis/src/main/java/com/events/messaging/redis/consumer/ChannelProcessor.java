...
public class ChannelProcessor {
  ...
  public ChannelProcessor(
    ...
    logger.info(
        "Channel processor is created (channel = {}, {})", channel, subscriptionIdentificationInfo);
  }

  public void process() {
    try {
      logger.info(
          "Channel processor started processing (channel = {}, {})",
          channel,
          subscriptionIdentificationInfo);
      running.set(true);
      ...
      logger.info(
          "Channel processor finished processing (channel = {}, {})",
          channel,
          subscriptionIdentificationInfo);
    } catch (Throwable e) {
      logger.error(
          "Channel processor got error: (channel = {}, subscriberId = {})", channel, subscriberId);
  ...
  public void stop() {
    ...
    try {
      ...
      logger.error(
          "Stopping channel failed (channel = {}, {})", channel, subscriptionIdentificationInfo);
  ...
  private boolean isKeyDoesNotExist(RedisSystemException e) {
    return isRedisCommandExceptionContainingMessage(
        e, "ERR The XGROUP subcommand requires the key to exist");
  }
  ...
  private boolean isRedisCommandExceptionContainingMessage(
      RedisSystemException e, String expectedMessage) {
  ...
  private void processMessage(String message, RecordId recordId) {
    logger.trace(
        "Channel processor {} with channel {} got message: {}",
        subscriptionIdentificationInfo,
        channel,
        message);
  ...
  private List<MapRecord<String, Object, Object>> getUnprocessedRecords() {
    return getRecords(
        ReadOffset.from(">"),
        StreamReadOptions.empty().block(Duration.ofMillis(blockStreamTimeInMilliseconds)));
  }

  private List<MapRecord<String, Object, Object>> getRecords(
      ReadOffset readOffset, StreamReadOptions options) {
    List<MapRecord<String, Object, Object>> records =
        redisTemplate
            .opsForStream()
            .read(
                Consumer.from(subscriberId, subscriberId),
                options,
