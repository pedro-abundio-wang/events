...
public class PartitionManager {
  ...
  public Map<String, Assignment> initialize(Map<String, Assignment> assignments) {
    ...
    Map<String, Assignment> reassignments =
        filterUnchangedAssignments(assignments, currentAssignments);

    logger.info(
        "Initializing rebalancer: assignments = {}, currentAssignments = {}, reassignments = {}",
        assignments,
        currentAssignments,
        reassignments);
  ...
  public Map<String, Assignment> rebalance(
      Map<String, Set<String>> addedGroupMembersWithTheirSubscribedChannels,
      Set<String> removedGroupMembers) {

    Map<String, Assignment> assignmentsWithoutRemovedMembers =
        currentAssignments.entrySet().stream()
            .filter(
                groupMemberAndAssignment ->
                    !removedGroupMembers.contains(groupMemberAndAssignment.getKey()))
            .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

    Map<String, Assignment> newMemberAssignments =
        addedGroupMembersWithTheirSubscribedChannels.entrySet().stream()
            .collect(
                Collectors.toMap(
                    Map.Entry::getKey,
                    groupMemberAndChannels ->
                        new Assignment(groupMemberAndChannels.getValue(), new HashMap<>())));
    ...
    Map<String, Assignment> changedAssignments =
        filterUnchangedAssignments(currentAssignments, reassignments);

    logger.info(
        "Rebalancing: addedGroupMembersWithTheirSubscribedChannels = {}, removedGroupMembers = {}, currentAssignments = {}, reassignments = {}",
        addedGroupMembersWithTheirSubscribedChannels,
        removedGroupMembers,
        currentAssignments,
        reassignments);
  ...
  private Map<String, Assignment> rebalance(Map<String, Assignment> assignments) {

    logger.info("Rebalancing assignments: {}", assignments);

    Set<AssignmentDescription> assignmentDescriptions =
    ...
    Map<String, Map<String, List<AssignmentDescription>>>
        assignmentDescriptionsByGroupMemberAndChannel =
            assignmentDescriptions.stream()
                .collect(Collectors.groupingBy(AssignmentDescription::getGroupMember))
                .entrySet()
                .stream()
                .collect(
                    Collectors.toMap(
                        Map.Entry::getKey, this::groupAssignmentDescriptionsByChannel));

    Map<String, Assignment> rebalancedAssignments =
        assignmentDescriptionsByGroupMemberAndChannel.entrySet().stream()
            .collect(
                Collectors.toMap(
                    Map.Entry::getKey, this::assignmentDescriptionsByChannelToAssignment));
  ...
  private Stream<AssignmentDescription> assignmentToAssignmentDescriptions(
      Map.Entry<String, Assignment> groupMemberAndAssignment) {
    String groupMember = groupMemberAndAssignment.getKey();
    Assignment assignment = groupMemberAndAssignment.getValue();

    return assignment.getChannels().stream()
        .map(
            channel ->
  ...
  private Map<String, List<AssignmentDescription>> groupAssignmentDescriptionsByChannel(
      Map.Entry<String, List<AssignmentDescription>> groupMemberAndAssignmentDescriptions) {
    List<AssignmentDescription> assignmentDescriptions =
        groupMemberAndAssignmentDescriptions.getValue();

    return assignmentDescriptions.stream()
        .collect(Collectors.groupingBy(AssignmentDescription::getChannel));
  }

  private Assignment assignmentDescriptionsByChannelToAssignment(
      Map.Entry<String, Map<String, List<AssignmentDescription>>>
          groupMemberAndAssignmentDescriptionsByChannel) {
    Map<String, List<AssignmentDescription>> assignmentDescriptionsByChannel =
        groupMemberAndAssignmentDescriptionsByChannel.getValue();

    Map<String, Set<Integer>> partitionsByChannel =
        assignmentDescriptionsByChannel.entrySet().stream()
            .collect(
                Collectors.toMap(
                    Map.Entry::getKey,
                    channelAndAssignmentDescriptions ->
                        channelAndAssignmentDescriptions.getValue().stream()
                            .flatMap(
                                assignmentDescription ->
                                    assignmentDescription.calculateExpectedPartitions().stream())
                            .collect(Collectors.toSet())));

    return new Assignment(assignmentDescriptionsByChannel.keySet(), partitionsByChannel);
  }

  private Map<String, Assignment> filterUnchangedAssignments(
      Map<String, Assignment> originalAssignments, Map<String, Assignment> reassignments) {
  ...
  private Set<AssignmentDescription> rebalance(Set<AssignmentDescription> assignmentDescriptions) {
    ...
    Set<AssignmentDescription> rebalancingDescriptions =
        assignmentDescriptions.stream().map(AssignmentDescription::new).collect(Collectors.toSet());

    Set<Integer> notActivePartitions = findNotActivePartitions(rebalancingDescriptions);

    notActivePartitions.forEach(
        partition ->
            findAssignmentDescriptionWithMinPartitions(rebalancingDescriptions)
                .assignPartition(partition));

    AssignmentDescription minPartitionAssignment =
        findAssignmentDescriptionWithMinPartitions(rebalancingDescriptions);
    AssignmentDescription maxPartitionAssignment =
        findAssignmentDescriptionWithMaxPartitions(rebalancingDescriptions);

    while (maxPartitionAssignment.calculateRebalancedPartitions()
            - minPartitionAssignment.calculateRebalancedPartitions()
        > 1) {
  ...
  private AssignmentDescription findAssignmentDescriptionWithMinPartitions(
      Set<AssignmentDescription> assignmentDescriptions) {
    return assignmentDescriptions.stream()
  ...
  private AssignmentDescription findAssignmentDescriptionWithMaxPartitions(
      Set<AssignmentDescription> assignmentDescriptions) {
    return assignmentDescriptions.stream()
  ...
  private static class AssignmentDescription {
    ...
    private final Set<Integer> currentPartitions;
    ...
    public AssignmentDescription(AssignmentDescription copy) {
      this(copy.getGroupMember(), copy.getChannel(), new HashSet<>(copy.currentPartitions));
    ...
    public AssignmentDescription(
        String groupMember, String channel, Set<Integer> currentPartitions) {
    ...
    public void takePartitionFrom(AssignmentDescription assignmentDescription) {
      Optional<Integer> partitionToReassign =
          assignmentDescription.assignedPartitions.stream().findAny();
      ...
      if (partitionToReassign.isPresent()) {
        ...
        partitionToReassign =
            assignmentDescription.calculateExpectedPartitions().stream().findAny();
